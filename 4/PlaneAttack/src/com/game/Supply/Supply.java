...
public class Supply implements Runnable{
    ...
    static {
        ...
        //修建爱心补给图片
        icon3 = new ImageIcon("src/img/爱心.png");
        Image temp3 = icon3.getImage().getScaledInstance(58, 58, icon3.getImage().SCALE_DEFAULT);
    ...
    public void run() {
        ...
        while (true){
           ...
           if(count%524288==0&&Main.supply_list.size()<1){
               //随机生成两种补给
               int num = random.nextInt(3)+1;//1 炸弹补给 2 子弹补给  3 爱心补给
               //生成一个随机数 位置
               int x = random.nextInt(390);//0-389
               //创建补给图片
               ...
               if(num==1){//炸弹补给
                   ...
                   label_supply.setBounds(x,-100, 58, 88);
               }else{
                   label_supply.setIcon(icon3);
                   label_supply.setBounds(x,-90, 58, 58);
               }
               //设置给补给实体
