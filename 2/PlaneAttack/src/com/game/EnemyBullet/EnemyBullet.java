...
public class EnemyBullet implements Runnable{
    ...
    static{
        ...
        Image temp = icon.getImage().getScaledInstance(5, 11, icon.getImage().SCALE_DEFAULT);
            ...
            if(isTrue){
                ...
                if(count%10==0&&Main.enemy_bullets_list.size()<Main.player_entity.level+3){
                    ...
                    if(Main.enemy_list.size()>0){
                        ...
                        if(Main.enemy_list.get(index).num==1){
                            label_bullet.setBounds(Main.enemy_list.get(index).label.getX()+28,Main.enemy_list.get(index).label.getY()+43,5,11);
                        }else if(Main.enemy_list.get(index).num==2){
                            label_bullet.setBounds(Main.enemy_list.get(index).label.getX()+35,Main.enemy_list.get(index).label.getY()+99,5,11);
                        }else{
                            label_bullet.setBounds(Main.enemy_list.get(index).label.getX()+55,Main.enemy_list.get(index).label.getY()+171,5,11);
                        }
                        //设置给敌人子弹实体
