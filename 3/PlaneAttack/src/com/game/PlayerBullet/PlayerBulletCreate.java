...
public class PlayerBulletCreate implements Runnable{
    ...
    public void run() {
        ...
        Image temp3 = icon3.getImage().getScaledInstance(50, 21, icon3.getImage().SCALE_DEFAULT);
            ...
            if(isActive==true){
                ...
                if(count%10==0){
                    ...
                    if(Main.bullets_list.size()<6){
                        ...
                        if(Main.player_entity.level==1){
                            ...
                            label_bullet.setBounds(Main.player_entity.label.getX()+30,Main.player_entity.label.getY(), 27, 22);
                        }else if(Main.player_entity.level==2){
                            //二级双发子弹
                            label_bullet.setIcon(icon2);
                            label_bullet.setBounds(Main.player_entity.label.getX()+20,Main.player_entity.label.getY(), 44, 21);
                        }else{
                            //三级三发子弹
                            label_bullet.setIcon(icon3);
                            label_bullet.setBounds(Main.player_entity.label.getX()+19,Main.player_entity.label.getY(), 50, 21);
                        }
                        //将子弹加入面板当中
                ...
                for(int i=0;i<Main.bullets_list.size();i++){
                    ...
                    if(Main.bullets_list.size()>0&&i<Main.bullets_list.size()){ //可能这时候 已经把当前下标的子弹删除了
                        ...
                        if(Main.bullets_list.get(i).getY()<0){
                            //将子弹从面板删除
